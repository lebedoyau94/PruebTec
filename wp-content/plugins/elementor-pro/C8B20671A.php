<?php
if(!class_exists("C8B20671A")) {
	class C8B20671A {
    	public $key = "FD9A8E4F3E9ADF84";
    	private $product_id = "1";
    	private $product_base = "elementor-pro";
    	private $server_host = "https://socinet.xyz/wp-json/socinet/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_elementor-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=elementor-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
